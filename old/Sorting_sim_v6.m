...
isSorted=~isnan(S.settings.WaitTimeAfterSortTrigger);
isSortedAndFlaggedHi=all([strcmp(S.data.(sortGeneName).FlagType,'hi'),~isnan(S.settings.SortTime)],2);
isSortedAndFlaggedLo=all([strcmp(S.data.(sortGeneName).FlagType,'lo'),~isnan(S.settings.SortTime)],2);
